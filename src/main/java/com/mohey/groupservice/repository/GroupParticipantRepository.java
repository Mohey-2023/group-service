...
public interface GroupParticipantRepository extends JpaRepository<GroupParticipantEntity, Long> {
	...
	@Query("SELECT gp FROM GroupParticipantEntity gp LEFT JOIN gp.groupParticipantStatusEntity ps WHERE gp.groupId = :groupId AND gp.memberUuid = :memberUuid AND (ps.createdDatetime IS NULL)")
